...
Весь путь оплаты подписки: проведение платежа через эквайера и фискализация чека по 54-ФЗ на отдельной дорожке – она асинхронна, чтобы чек «догонял» платёж и не блокировал доступ к курсу.

![BPMN – оплата подписки с фискализацией](diagrams/rendered/bpmn_payment_end_to_end.png)

### C4 – System Context

Место сервиса в экосистеме онлайн-школы: оплату инициирует LMS, события уходят в бухгалтерию и уведомления по Kafka, а внешние интеграции – эквайер (PSP) для карточных операций и ОФД для фискальных чеков.

![C4 System Context](diagrams/rendered/c4_payment.png)

### C4 – Container

Внутреннее устройство сервиса: доменное ядро с конечным автоматом платежа и подписками, адаптеры эквайера и ОФД как ACL с Circuit Breaker + Retry, Scheduler для рекуррента/dunning/автоотмены холда и Outbox для надёжных событий; карточные данные в сервисе не хранятся.
...
Особенность – развязка платежа и фискализации при сбое: вебхук оплаты подтверждается сразу, а при обрыве связи с ОФД срабатывает Circuit Breaker – платёж не откатывается, чек регистрируется повтором позже.

![Sequence – вебхук при сбое](diagrams/rendered/sequence_payment_webhook_failure.png)

### ER-модель данных

Модель данных платежей: на один `payment` приходится несколько `transaction` (авторизация/капчура/возврат/чарджбэк), отдельно ведутся фискальные чеки (`receipt`) и чарджбэки; вместо карты хранится токен эквайера (PCI DSS), события – через Inbox/Outbox.
